...
def choose_jobs(q, jobs):
    jobs_set = BST()
    result = 0
    for i in range(q):
        today_jobs = jobs[i][1]
        for j in range(jobs[i][0]):
            jobs_set.insert(today_jobs[j] + i)
        min_val = jobs_set.getMinimum()
        if min_val is not None:
            result += 1
        jobs_set.delete(min_val)
        while jobs_set.has(i + 1):
            jobs_set.delete(i + 1)
    return result
