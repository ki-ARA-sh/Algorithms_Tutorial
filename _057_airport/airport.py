...
def program_airport(q, k, planes):
    planes_bst = BST()
    result = [''] * q
    for i in range(q):
        x = planes_bst.maximumNumberLessThan(planes[i])
        y = planes_bst.minimumNumberGreaterThan(planes[i])
        granted = True
        if planes_bst.has(planes[i]):
            granted = False
        if granted and (x is not None):
            if abs(planes[i] - x) < k:
                granted = False
        if granted and (y is not None):
            if abs(y - planes[i]) < k:
                granted = False
        if not granted:
            result[i] = 'Permission Denied!'
        else:
            result[i] = 'Permission Granted!'
            planes_bst.insert(planes[i])
    return result
...
def main():
    ...
    for i in range(q):
        planes[i] = int(input())
    land_points = program_airport(q, k, planes)
    print('\n'.join(land_points))
